...
export const DateFilter: React.FC<DateFilterProps> = ({ selectedDate, onSelectDate, activeSport }) => {
    ...
    return (
        <div className="w-full bg-slate-100 dark:bg-[#050a05] border-b border-white/5 dark:border-white/5 relative mb-4">
                            ...
                            return (
                                <button
                                    ...
                                    className={`relative flex flex-col items-center justify-center py-2 px-3 rounded-lg transition-all duration-150 min-w-[60px]
                                        ${isSelected
                                            ? 'bg-primary/10 border border-primary/30 text-white dark:text-white'
                                            : past
                                                ? 'text-slate-500 dark:text-slate-600 border border-transparent hover:border-slate-300 dark:hover:border-white/5'
                                                : 'text-slate-700 dark:text-text-muted hover:text-black dark:hover:text-text-main border border-transparent hover:border-slate-300 dark:hover:border-white/5'
                                        }`}
                                >
                                    {/* Selected top bar */}
                                    {isSelected && (
                                            ...
                                            {hasGames ? (
                                                <>
                                                    {Array.from({ length: Math.min(count, 5) }).map((_, i) => (
                                                        <div
                                                            key={i}
                                                            className={`w-1 h-1 rounded-full ${isSelected ? 'bg-primary' : past ? 'bg-slate-400 dark:bg-slate-700' : 'bg-slate-600 dark:bg-slate-500'}`}
                                                        ></div>
                                                    ))}
                                                    {count > 5 && (
                                                        <span className="text-[8px] text-slate-500 dark:text-slate-600 font-bold">+</span>
                                                    )}
                                                </>
                                            ) : (
                                    ...
                                    {loadingCounts && (
                                        <div className="w-4 h-1 mt-1 bg-slate-300 dark:bg-neutral-800 rounded animate-pulse"></div>
                                    )}
                                </button>
                            );
                        })}
            ...
            {/* Fade edges */}
            <div className="absolute left-0 top-0 bottom-0 w-8 bg-gradient-to-r from-slate-100 dark:from-[#050a05] to-transparent pointer-events-none"></div>
            <div className="absolute right-0 top-0 bottom-0 w-8 bg-gradient-to-l from-slate-100 dark:from-[#050a05] to-transparent pointer-events-none"></div>
        </div>
    );
};
