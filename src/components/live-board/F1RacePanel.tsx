...
// Walk back up to 30 days to find a recent F1 race
async function fetchMostRecentF1Race(daysBack = 30): Promise<F1Race | null> {
    for (let d = 0; d <= daysBack; d++) {
        ...
        try {
            ...
            const proxyResponse = await fetchFromBackend(`/racing/f1/scoreboard?dates=${dateStr}`);
            ...
            const drivers: F1Driver[] = competitors.map((c: RawObj, idx: number) => {
            ...
            return {
                id: ev?.id ?? '',
                name: ev?.name ?? 'Formula 1 Grand Prix',
...
function driverToGame(driver: F1Driver, race: F1Race): Game {
    ...
    return {
        ...
        sportLogo: 'https://a.espncdn.com/i/teamlogos/leagues/500/f1.png',
        broadcast: '',
...
interface F1RacePanelProps {
    onSelectGame?: (game: Game) => void;
}

export const F1RacePanel: React.FC<F1RacePanelProps> = ({ onSelectGame }) => {
    ...
    const load = useCallback(async () => {
        ...
        setError(null);
        try {
            const r = await fetchMostRecentF1Race(30);
            if (r) setRace(r);
            else setError('No recent F1 races found.');
        } catch { setError('Failed to load F1 data.'); }
    ...
    if (error || !race) return (
        <div className="flex flex-col items-center justify-center gap-4 py-16 text-center">
            ...
            <p className="text-slate-500 text-sm font-bold uppercase tracking-widest">No F1 races found</p>
        </div>
    );
