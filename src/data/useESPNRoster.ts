...
export const useESPNRoster = (teamName: string, sport: string, team?: { id: string; leagueSlug?: string; coreSport?: string; seasonYear?: number; }) => {
    ...
    const load = useCallback(async () => {
        ...
        setError(null);
        try {
            const data = await fetchESPNRosterBySport(teamName, sport, team);
            if (data.length === 0) {
