...
export const fetchESPNRosterBySport = async (teamName: string, sport: string, teamMeta?: { id: string; leagueSlug?: string; coreSport?: string; seasonYear?: number; }): Promise<ESPNRosterAthlete[]> => {
    // If teamMeta provides the exact ESPN routing data, use it directly!
    // This effortlessly supports new leagues like Soccer and Baseball.WBC without hardcoded ID maps
    if (teamMeta && teamMeta.id && teamMeta.leagueSlug && teamMeta.coreSport) {
        const teamId = teamMeta.id;
        const season = teamMeta.seasonYear || new Date().getFullYear();
        const rosterUrl = `https://sports.core.api.espn.com/v2/sports/${teamMeta.coreSport}/leagues/${teamMeta.leagueSlug}/seasons/${season}/teams/${teamId}/athletes?limit=60`;
        ...
        try {
            ...
            const limit = teamMeta.coreSport === 'football' ? 30 : 25;
            ...
            console.warn(`ESPN roster fetch failed for ${teamName} using exact slugs:`, err);
            return [];
        }
    }
